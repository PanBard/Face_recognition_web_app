  ...
  public async make_request_to_api() : Promise<void>
  {
    if(this.check_data_before_api())
    {
      this.turn_off_on_loading_circle();
        ...
        try 
        {
          const data ={ Name: this.name, Password: this.password, ImageString: this.image_base64 };
          const response = await this.http.post(this.api_url+'/database/addNewProfile', data, httpOptions).toPromise();
